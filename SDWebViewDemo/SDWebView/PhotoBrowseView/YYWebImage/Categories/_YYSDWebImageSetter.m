...
NSString *const _YYWebImageFadeAnimationKey = @"YYSDWebImageFade";
...
- (int32_t)setOperationWithSentinel:(int32_t)sentinel
    ...
    if (sentinel != _sentinel) {
        if (completion) completion(nil, imageURL, YYSDWebImageFromNone, YYSDWebImageStageCancelled, nil);
        return _sentinel;
    }
    ...
    if (!operation && completion) {
        NSDictionary *userInfo = @{ NSLocalizedDescriptionKey : @"YYSDWebImageOperation create failed." };
        completion(nil, imageURL, YYSDWebImageFromNone, YYSDWebImageStageFinished, [NSError errorWithDomain:@"com.ibireme.webimage" code:-1 userInfo:userInfo]);
    }
