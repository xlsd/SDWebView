...
typedef NS_ENUM(NSUInteger, YYSDAnimatedImageType) {
    YYSDAnimatedImageTypeNone = 0,
    YYSDAnimatedImageTypeImage,
    YYSDAnimatedImageTypeHighlightedImage,
    YYSDAnimatedImageTypeImages,
    YYSDAnimatedImageTypeHighlightedImages,
};

@interface YYSDAnimatedImageView() {
    @package
    UIImage <YYSDAnimatedImage> *_curAnimatedImage;
...
@property (nonatomic, weak) YYSDAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <YYSDAnimatedImage> *curImage;
@end

@implementation _YYAnimatedImageViewFetchOperation
- (void)main {
    __strong YYSDAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation YYSDAnimatedImageView
...
- (void)setImage:(UIImage *)image {
    ...
    [self setImage:image withType:YYSDAnimatedImageTypeImage];
}

- (void)setHighlightedImage:(UIImage *)highlightedImage {
    if (self.highlightedImage == highlightedImage) return;
    [self setImage:highlightedImage withType:YYSDAnimatedImageTypeHighlightedImage];
}

- (void)setAnimationImages:(NSArray *)animationImages {
    if (self.animationImages == animationImages) return;
    [self setImage:animationImages withType:YYSDAnimatedImageTypeImages];
}
...
- (void)setHighlightedAnimationImages:(NSArray *)highlightedAnimationImages {
    ...
    [self setImage:highlightedAnimationImages withType:YYSDAnimatedImageTypeHighlightedImages];
}
...
- (id)imageForType:(YYSDAnimatedImageType)type {
    switch (type) {
        case YYSDAnimatedImageTypeNone: return nil;
        case YYSDAnimatedImageTypeImage: return self.image;
        case YYSDAnimatedImageTypeHighlightedImage: return self.highlightedImage;
        case YYSDAnimatedImageTypeImages: return self.animationImages;
        case YYSDAnimatedImageTypeHighlightedImages: return self.highlightedAnimationImages;
    }
    return nil;
}

- (YYSDAnimatedImageType)currentImageType {
    YYSDAnimatedImageType curType = YYSDAnimatedImageTypeNone;
    if (self.highlighted) {
        if (self.highlightedAnimationImages.count) curType = YYSDAnimatedImageTypeHighlightedImages;
        else if (self.highlightedImage) curType = YYSDAnimatedImageTypeHighlightedImage;
    }
    if (curType == YYSDAnimatedImageTypeNone) {
        if (self.animationImages.count) curType = YYSDAnimatedImageTypeImages;
        else if (self.image) curType = YYSDAnimatedImageTypeImage;
    }
    return curType;
}

- (void)setImage:(id)image withType:(YYSDAnimatedImageType)type {
    [self stopAnimating];
    if (_link) [self resetAnimated];
    _curFrame = nil;
    switch (type) {
        case YYSDAnimatedImageTypeNone: break;
        case YYSDAnimatedImageTypeImage: super.image = image; break;
        case YYSDAnimatedImageTypeHighlightedImage: super.highlightedImage = image; break;
        case YYSDAnimatedImageTypeImages: super.animationImages = image; break;
        case YYSDAnimatedImageTypeHighlightedImages: super.highlightedAnimationImages = image; break;
    }
    [self imageChanged];
}

- (void)imageChanged {
    YYSDAnimatedImageType newType = [self currentImageType];
    id newVisibleImage = [self imageForType:newType];
    NSUInteger newImageFrameCount = 0;
    BOOL hasContentsRect = NO;
    if ([newVisibleImage isKindOfClass:[UIImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(YYSDAnimatedImage)]) {
        newImageFrameCount = ((UIImage<YYSDAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((UIImage<YYSDAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((UIImage<YYSDAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)startAnimating {
    YYSDAnimatedImageType type = [self currentImageType];
    if (type == YYSDAnimatedImageTypeImages || type == YYSDAnimatedImageTypeHighlightedImages) {
...
- (void)step:(CADisplayLink *)link {
    UIImage <YYSDAnimatedImage> *image = _curAnimatedImage;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    UIImage *image = [aDecoder decodeObjectForKey:@"YYSDAnimatedImage"];
    UIImage *highlightedImage = [aDecoder decodeObjectForKey:@"YYSDHighlightedAnimatedImage"];
    if (image) {
        self.image = image;
        [self setImage:image withType:YYSDAnimatedImageTypeImage];
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(YYSDAnimatedImage)];
    multi = (ani && ((UIImage <YYSDAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"YYSDAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(YYSDAnimatedImage)];
    multi = (ani && ((UIImage <YYSDAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.highlightedImage forKey:@"YYSDHighlightedAnimatedImage"];
}
