...
/// Image cache type
typedef NS_OPTIONS(NSUInteger, YYSDImageCacheType) {
    /// No value.
    YYSDImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    YYSDImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    YYSDImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    YYSDImageCacheTypeAll    = YYSDImageCacheTypeMemory | YYSDImageCacheTypeDisk,
};


/**
 YYSDImageCache is a cache that stores UIImage and image data based on memory cache and disk cache.
...
/**
 ...
 @discussion When fetch image from disk cache, it will use 'YYSDImage' to decode 
...
/**
 ...
 @return  The singleton YYSDImageCache instance.
 */
...
/**
 ...
 @discussion If the `type` contain `YYSDImageCacheTypeMemory`, then the `image` will 
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `YYSDImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(YYSDImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `YYSDImageCacheTypeDisk`,
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(YYSDImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(YYSDImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, YYSDImageCacheType type))block;
