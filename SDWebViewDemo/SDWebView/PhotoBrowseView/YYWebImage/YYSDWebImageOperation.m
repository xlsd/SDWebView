...
static BOOL YYSDCGImageLastPixelFilled(CGImageRef image) {
    if (!image) return NO;
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    if (width == 0 || height == 0) return NO;
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, YYSDCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface YYSDWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) YYSDImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) YYSDWebImageProgressBlock progress;
@property (nonatomic, copy) YYSDWebImageTransformBlock transform;
@property (nonatomic, copy) YYSDWebImageCompletionBlock completion;
@end


@implementation YYSDWebImageOperation
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"YYSDWebImageOperation init error" reason:@"YYSDWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            ...
            if (![_request.URL isFileURL] && (_options & YYSDWebImageOptionShowNetworkActivity)) {
                [YYSDWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, YYSDWebImageFromNone, YYSDWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & YYSDWebImageOptionUseNSURLCache) &&
            !(_options & YYSDWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:YYSDImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, YYSDWebImageFromMemoryCache, YYSDWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & YYSDWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:YYSDImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:YYSDImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & YYSDWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, YYSDWebImageFromNone, YYSDWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, YYSDWebImageFromNone, YYSDWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, YYSDWebImageFromDiskCache, YYSDWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & YYSDWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([YYSDWebImageOperation _imageQueue], ^{
                        [self->_cache setImage:image imageData:data forKey:self->_cacheKey withType:YYSDImageCacheTypeAll];
                    });
                }
            }
            _data = nil;
            ...
            if (!image) {
                ...
                if (_options & YYSDWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, YYSDWebImageFromRemote, YYSDWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & YYSDWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & YYSDWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & YYSDWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & YYSDWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[YYSDImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == YYSDImageTypeUnknown ||
            _progressiveDecoder.type == YYSDImageTypeWebP ||
            _progressiveDecoder.type == YYSDImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != YYSDImageTypeJPEG &&
                _progressiveDecoder.type != YYSDImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            YYSDImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, YYSDWebImageFromRemote, YYSDWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == YYSDImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (!YYSDCGImageLastPixelFilled(image.CGImage)) return;
            ...
            image = [image YYSD_imageByBlurRadius:radius tintColor:nil tintMode:0 saturation:1 maskImage:nil];
            
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, YYSDWebImageFromRemote, YYSDWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & YYSDWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & YYSDWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[YYSDImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image yysd_imageByDecoded];
                    if ([((YYSDImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    YYSDImageDecoder *decoder = [YYSDImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                YYSDImageType imageType = YYSDImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case YYSDImageTypeJPEG:
                    case YYSDImageTypeGIF:
                    case YYSDImageTypePNG:
                    case YYSDImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == YYSDImageTypeGIF ||
                                imageType == YYSDImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & YYSDWebImageOptionShowNetworkActivity)) {
                [YYSDWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, YYSDWebImageFromNone, YYSDWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & YYSDWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & YYSDWebImageOptionAllowBackgroundTask) && _YYSharedApplication()) {
