...
/**
 YYSDKVStorageItem is used by `YYSDKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface YYSDKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `YYSDKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use YYSDKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use YYSDKVStorageTypeFile to get better performance.
 * You can use YYSDKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, YYSDKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    YYSDKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    YYSDKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    YYSDKVStorageTypeMixed = 2,
};
 ...
 */
@interface YYSDKVStorage : NSObject
...
@property (nonatomic, readonly) YYSDKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(YYSDKVStorageType)type NS_DESIGNATED_INITIALIZER;
 ...
 */
- (BOOL)saveItem:(YYSDKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 YYSDKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is YYSDKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is YYSDKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is YYSDKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable YYSDKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable YYSDKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `YYSDKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<YYSDKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `YYSDKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<YYSDKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
