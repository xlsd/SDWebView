...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"YYSDDiskCache init error" reason:@"YYSDDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    if (threshold == 0) {
        type = YYSDKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = YYSDKVStorageTypeSQLite;
    } else {
        type = YYSDKVStorageTypeMixed;
    }
    
    YYSDKVStorage *kv = [[YYSDKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    YYSDKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [YYSDDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [YYSDDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != YYSDKVStorageTypeSQLite) {
