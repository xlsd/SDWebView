...
+ (YYSDImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (YYSDImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (YYSDImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        YYSDImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
        ...
        self.yysd_isDecodedForDisplay = YES;
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"YYSDImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"YYSDImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"YYSDImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"YYSDImageData"];
    } else {
        [super encodeWithCoder:aCoder]; // Apple use UIImagePNGRepresentation() to encode UIImage.
    }
}

#pragma mark - protocol YYSDAnimatedImage
