...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `YYSDAnimatedImage` protocol,
...
/**
 The YYSDAnimatedImage protocol declares the required methods for animated image
 display with YYSDAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to YYSDAnimatedImageView.image or YYSDAnimatedImageView.highlightedImage
 to display animation.
 
 See `YYSDImage` and `YYSDFrameImage` for example.
 */
@protocol YYSDAnimatedImage <NSObject>
@required
