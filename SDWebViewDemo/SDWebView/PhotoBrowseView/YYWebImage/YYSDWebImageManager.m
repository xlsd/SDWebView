...
+ (instancetype)sharedManager {
    static YYSDWebImageManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        YYSDImageCache *cache = [YYSDImageCache sharedCache];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"YYSDWebImageManager init error" reason:@"Use the designated initializer to init." userInfo:nil];
    return [self initWithCache:nil queue:nil];
}

- (instancetype)initWithCache:(YYSDImageCache *)cache queue:(NSOperationQueue *)queue{
    self = [super init];
    if (!self) return nil;
    _cache = cache;
    _queue = queue;
    _timeout = 15.0;
    if (YYSDImageWebPAvailable()) {
...
- (YYSDWebImageOperation *)requestImageWithURL:(NSURL *)url
    ...
    request.HTTPShouldHandleCookies = (options & YYSDWebImageOptionHandleCookies) != 0;
    request.allHTTPHeaderFields = [self headersForURL:url];
    request.HTTPShouldUsePipelining = YES;
    request.cachePolicy = (options & YYSDWebImageOptionUseNSURLCache) ?
        NSURLRequestUseProtocolCachePolicy : NSURLRequestReloadIgnoringLocalCacheData;
    
    YYSDWebImageOperation *operation = [[YYSDWebImageOperation alloc] initWithRequest:request
                                                                          options:options
                                                                            cache:_cache
