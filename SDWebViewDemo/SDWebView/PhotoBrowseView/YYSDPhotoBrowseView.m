...
#import "UIView+YYSDAdd.h"
#import "CALayer+YYSDAdd.h"
#import "YYSDCGUtilities.h"
#define kPadding 20
#define kSystemVersion [[UIDevice currentDevice].systemVersion doubleValue]

#ifndef YYSD_CLAMP // return the clamped value
#define YYSD_CLAMP(_x_, _low_, _high_)  (((_x_) > (_high_)) ? (_high_) : (((_x_) < (_low_)) ? (_low_) : (_x_)))
#endif
...
- (id)copyWithZone:(NSZone *)zone {
    YYSDPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
@interface YYSDPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
@property (nonatomic, strong) UIView *imageContainerView;
@property (nonatomic, strong) YYSDAnimatedImageView *imageView;
...
@property (nonatomic, strong) YYSDPhotoGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation YYSDPhotoGroupCell
...
- (instancetype)init {
    ...
    _imageView = [YYSDAnimatedImageView new];
...
- (void)setItem:(YYSDPhotoGroupItem *)item {
    ...
    [_imageView YYSD_cancelCurrentImageRequest];
    ...
    [_imageView YYSD_setImageWithURL:item.largeImageURL placeholder:item.thumbImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self);
        if (!self) return;
    ...
    } transform:nil completion:^(UIImage *image, NSURL *url, YYSDWebImageFromType from, YYSDWebImageStage stage, NSError *error) {
        @strongify(self);
        if (!self) return;
        self.progressLayer.hidden = YES;
        if (stage == YYSDWebImageStageFinished) {
            self.maximumZoomScale = 3;
            if (image) {
...
@interface YYSDPhotoBrowseView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation YYSDPhotoBrowseView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        ...
        if([fromView isEqual:((YYSDPhotoGroupItem *)self.groupItems[i]).thumbView]){
            page = (int)i;
            break;
        }
    }
    ...
    if (_blurEffectBackground) {
        _blurBackground.image = [_snapshorImageHideFromView YYSD_imageByBlurDark]; //Same to UIBlurEffectStyleDark
    } else {
        _blurBackground.image = [UIImage YYSD_imageWithColor:[UIColor blackColor]];
    }
...
#pragma clang diagnostic pop
    ...
    YYSDPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    YYSDPhotoGroupItem *item = _groupItems[self.currentPage];
    
    if (!item.thumbClippedToTop) {
        NSString *imageKey = [[YYSDWebImageManager sharedManager] cacheKeyForURL:item.largeImageURL];
        if ([[YYSDWebImageManager sharedManager].cache getImageForKey:imageKey withType:YYSDImageCacheTypeMemory]) {
            cell.item = item;
        }
    }
    if (!cell.item) {
...
#pragma clang diagnostic pop
    ...
    YYSDPhotoGroupCell *cell = [self cellForPage:currentPage];
    YYSDPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(YYSDPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
        [cell.imageView YYSD_cancelCurrentImageRequest];
    }];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            YYSDPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                YYSDPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (YYSDPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (YYSDPhotoGroupCell *)dequeueReusableCell {
    YYSDPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (YYSDPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (YYSDPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    YYSDPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
...
- (void)saveImage {
    ...
    id imageItem = [tile.imageView.image yysd_imageDataRepresentation];
    YYSDImageType type = YYSDImageDetectType((__bridge CFDataRef)(imageItem));
    if (type != YYSDImageTypePNG &&
        type != YYSDImageTypeJPEG &&
        type != YYSDImageTypeGIF) {
...
//                [[UIApplication sharedApplication] setStatusBarHidden:_fromNavigationBarHidden withAnimation:UIStatusBarAnimationFade];
                ...
                duration *= 0.8;
                duration = YYSD_CLAMP(duration, 0.05, 0.3);
