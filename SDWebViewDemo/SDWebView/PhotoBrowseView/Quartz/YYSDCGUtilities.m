...
CGContextRef YYSDCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef YYSDCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform YYSDCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform YYSDCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    return YYSDCGAffineTransformGetFromPoints(before, after);
}
